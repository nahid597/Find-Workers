...
function starmark(item) {
    ...
    console.log("chek rating bar" + ratingBarCheck);
    ...
    console.log("rating number " + count);
...
function submitStars() {
    ...
    var dbrating = this.storeDbElements[0].Rating.rating;
    var dbRatingCount = this.storeDbElements[0].Rating.count;
    //  console.log(dbrating);
    //  console.log(dbRatingCount);
    stroeDbRatingPerPerson[0] = dbrating;
...
function confirmToWorker(id1) {
    ...
    console.log("hello " + this.dbElementsCount);

    for (let [key, value] of Object.entries(marker)) {
        //console.log(" in side loop...");
        if (this.storeDbElements.some(item => item._id === key) == false) {
    ...
    if (id1) {
        this.client.get('http://localhost:4487/admin/workers?_id=' + id1, function(response) {
            ...
            //console.log(store[1].Coordinate.x);
            this.dbElementsCount = this.storeDbElements.length;
            console.log("numerb after select: " + dbElementsCount);
...
// var urlCategoty = document.location.search.replace(/^.*?\=/, '');
// console.log('url' + urlCategoty);
// if (urlCategoty !== '') {
//     document.getElementById('singleSelectTextDDJS').value = urlCategoty;
// }
...
function singleSelectChangeText() {
    ...
    console.log("Size: " + this.marker.length);
    ...
    console.log("Size: " + marker.length);
